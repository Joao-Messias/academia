...
function cadastrarAssinatura() {
  ...
  let body = {
    ...
    nomePlano: document.getElementById("plano").value,
  };

  fetch(url + "assinatura/cadastrar", {
    method: "POST",
    redirect: "follow",
    headers: {
      "Content-Type": "application/json",
  ...
  fetch(url + "clientes")
    ...
    .then((clientes) => {
      ...
      for (let cliente of clientes) {
        ...
        btnAtualizar.innerHTML = "Atualizar";
        btnAtualizar.onclick = (u) =>
          atualizarCliente(cliente.id, divNome, divEmail, divCpf, divTelefone);
        btnAtualizar.style.marginLeft = "5px";
...
function atualizarCliente(id, divNome, divEmail, divCpf, divTelefone) {
  let body = {
    nome: divNome.value,
